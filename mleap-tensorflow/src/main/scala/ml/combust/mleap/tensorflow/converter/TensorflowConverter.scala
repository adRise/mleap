  ...
  def convert(tensor: tensorflow.Tensor, tensorType: TensorType): DenseTensor[_] = {
    ...
    tensor match {
      case u8: TUint8 =>
        ...
        u8.asRawTensor().data().read(buffer.array())
        DenseTensor(buffer.array, dimensions)
      case i32: TInt32 =>
        val buffer = IntBuffer.allocate(size)
        i32.asRawTensor().data().asInts().read(buffer.array())
        DenseTensor(buffer.array, dimensions)
      case i64: TInt64 =>
        val buffer = LongBuffer.allocate(size)
        i64.asRawTensor().data().asLongs().read(buffer.array())
        DenseTensor(buffer.array, dimensions)
      case f32: TFloat32 =>
        val buffer = FloatBuffer.allocate(size)
        f32.asRawTensor().data().asFloats().read(buffer.array())
        DenseTensor(buffer.array, dimensions)
      case f64: TFloat64 =>
        val buffer = DoubleBuffer.allocate(size)
        f64.asRawTensor().data().asDoubles().read(buffer.array())
        DenseTensor(buffer.array, dimensions)
      case str: TString =>
